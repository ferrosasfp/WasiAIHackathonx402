...
import { getReputationRegistryAddress, getChainConfig, DEFAULT_CHAIN_ID } from '@/config/chains'

// Configuration - uses centralized chain config (no hardcoded values)
const CHAIN_ID = DEFAULT_CHAIN_ID
const CHAIN_CONFIG = getChainConfig(CHAIN_ID)
...
// Create viem client for blockchain reads using centralized config
function getClient() {
  if (!CHAIN_CONFIG) {
