  ...
  useEffect(() => {
    ...
    const loadExistingModel = async () => {
      try {
        ...
        console.log('[Step3 Upgrade] Full API response:', {
          hasModel: !!modelData,
          modelFields: modelData ? Object.keys(modelData) : [],
          hasMetadata: !!meta,
        ...
        // Load download notes from multiple possible locations
        // Priority: demo.downloadNotes > root downloadNotes > customer.downloadInstructions
        const loadedDownloadNotes = meta?.demo?.downloadNotes || 
                                    meta?.downloadNotes || 
                                    meta?.customer?.downloadInstructions ||
                                    meta?.downloadInstructions || ''
        console.log('[Step3 Upgrade] Download notes sources:', {
          'meta.demo.downloadNotes': meta?.demo?.downloadNotes,
          'meta.downloadNotes': meta?.downloadNotes,
          'meta.customer.downloadInstructions': meta?.customer?.downloadInstructions,
          'final': loadedDownloadNotes
        })
        setDownloadNotes(loadedDownloadNotes)
        
        // Debug: log all possible sources
        console.log('[Step3 Upgrade] Model data fields:', {
          agent_endpoint: modelData?.agent_endpoint,      // From agents table (priority)
          agent_wallet: modelData?.agent_wallet,          // From agents table (priority)
          inference_endpoint: modelData?.inference_endpoint, // From models table
          inference_wallet: modelData?.inference_wallet,     // From models table
        })
        console.log('[Step3 Upgrade] Metadata sources:', {
          'meta.demo': meta?.demo,
          'meta.inferenceConfig': meta?.inferenceConfig,
          'meta.inference': meta?.inference,
        })
        
        // Load inference config from model data or metadata
        // Priority: agent fields (from agents table) > model fields > metadata
        // This matches the same priority used in /api/indexed/models listing
        const inferenceEp = modelData?.agent_endpoint ||  // From agents table (ERC-8004)
                           modelData?.inference_endpoint || // From models table
                           meta?.demo?.endpoint || 
                           meta?.inferenceConfig?.endpoint || 
                           meta?.inference?.endpoint || ''
        const inferenceWallet = modelData?.agent_wallet ||  // From agents table
                               modelData?.inference_wallet || // From models table
                               meta?.demo?.paymentWallet || 
                               meta?.inferenceConfig?.paymentWallet || 
                               meta?.inference?.wallet || ''
        
        // Always set, even if empty (to clear stale state)
        setInferenceEndpoint(inferenceEp)
        setInferencePaymentWallet(inferenceWallet)
        
        console.log('[Step3 Upgrade] Final inference config:', { inferenceEp, inferenceWallet })
        
        // IMPORTANT: Save loaded data to draft so Step 5 has access to it
        // This ensures inference config is available even if user doesn't edit Step 3
        const loadedArtifacts = meta?.artifacts && Array.isArray(meta.artifacts) 
          ...
          : [{ cid: '', filename: '', role: 'other' as ArtifactRole }]
        
        // Save to draft in background (non-blocking) so Step 5 can access it
        // Don't await - let UI update immediately
        saveStepCentralized('step3', {
          artifacts: loadedArtifacts,
          downloadNotes: loadedDownloadNotes,
          inferenceConfig: {
            endpoint: inferenceEp,
            paymentWallet: inferenceWallet
          }
        }, upgradeMode, upgradeModelId)
          .then(() => console.log('[Step3 Upgrade] Saved loaded data to draft'))
          .catch((saveErr) => console.warn('[Step3 Upgrade] Failed to save loaded data to draft:', saveErr))
