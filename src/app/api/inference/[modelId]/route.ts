...
import { recordInferenceAsync } from '@/lib/inference-history'
import {
  ACTIVE_CHAIN,
  USDC_DECIMALS,
  DEFAULT_FACILITATOR_URL,
  DEFAULT_PRICE_USDC_BASE_UNITS,
  MAX_TIMEOUT_SECONDS,
  RATE_LIMIT,
  HUGGINGFACE,
  formatUsdcPrice,
...
import { getFacilitator, getActiveProvider } from '@/lib/x402-facilitators'

export const dynamic = 'force-dynamic'

// ===== x402 Protocol Configuration =====
// Using shared constants from @/lib/x402-constants

const CHAIN_ID = ACTIVE_CHAIN.chainId
const NETWORK = ACTIVE_CHAIN.network
const USDC_ADDRESS = process.env.X402_USDC_ADDRESS || getUsdcAddress(CHAIN_ID)
...
// Default price per inference in USDC base units
const DEFAULT_PRICE_USDC = process.env.X402_DEFAULT_PRICE_USDC || DEFAULT_PRICE_USDC_BASE_UNITS
...
// Note: Inference history is now persisted to Neon DB via @/lib/inference-history
// See: src/lib/inference-history.ts for CRUD operations
...
function rateLimit(key: string, max = RATE_LIMIT.maxRequests, windowMs = RATE_LIMIT.windowMs) {
  const now = Date.now()
...
// 
// Facilitator providers are abstracted in @/lib/x402-facilitators
// Switch providers via X402_FACILITATOR_PROVIDER env variable:
// - 'ultravioleta' (default, free, Avalanche only)
// - 'thirdweb' (paid, multi-chain)
// - 'mock' (testing, no blockchain)
//
...
): Promise<{ valid: boolean; error?: string; txHash?: string; payer?: string; provider?: string }> {
  try {
    ...
    // Use the facilitator abstraction layer
    const facilitator = getFacilitator()
    const provider = getActiveProvider()
    
    console.log(`[x402] Using ${provider} facilitator for resource: ${requirement.resource}`)
    
    const result = await facilitator.settle(payment, requirement)
    
    if (!result.success) {
      console.error(`[x402] Settlement failed via ${provider}:`, result.error)
      return { 
        valid: false, 
        error: result.error,
        provider,
...
const HF_API_URL = HUGGINGFACE.apiUrl
const HF_TOKEN = process.env.HUGGINGFACE_API_TOKEN
const HF_TIMEOUT_MS = HUGGINGFACE.timeoutMs
const HF_MAX_RETRIES = HUGGINGFACE.maxRetries

// Fetch with timeout and retry for HuggingFace API
async function fetchWithRetry(
  url: string, 
  options: RequestInit, 
  retries: number = HF_MAX_RETRIES
): Promise<Response> {
...
// formatUsdcPrice imported from @/lib/x402-constants
...
export async function POST(
  ...
  // Record inference in history (async - fire-and-forget to DB)
  const inputStr = typeof body.input === 'string' ? body.input : JSON.stringify(body.input || '')
  const outputStr = typeof result.output === 'string' ? result.output : JSON.stringify(result.output || '')
  
  // Non-blocking DB insert - doesn't delay API response
  // Use agentId from request body (frontend) if available, fallback to modelInfo
  const effectiveAgentId = body.agentId || modelInfo.agentId
  recordInferenceAsync({
    modelId,
    modelName: modelInfo.name,
    agentId: effectiveAgentId,
    payer: verification.payer || '',
    txHash: verification.txHash || null,
    amountUsdc: modelInfo.pricePerInference,
