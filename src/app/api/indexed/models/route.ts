  ...
  try {
    ...
    const dataQuery = `
      SELECT DISTINCT ON (COALESCE(m.slug, m.model_id::text), LOWER(m.owner))
        ...
        mm.architectures,
        mm.cached_at,
        -- Use agent_id from models table (already indexed), fallback to agents table
        COALESCE(m.agent_id, a.agent_id) as agent_id,
        a.wallet as agent_wallet,
        a.endpoint as agent_endpoint,
        a.active as agent_active
      FROM models m
      LEFT JOIN model_metadata mm ON m.model_id = mm.model_id
      LEFT JOIN agents a ON a.model_id = m.model_id
      ${whereClause}
      ORDER BY COALESCE(m.slug, m.model_id::text), LOWER(m.owner), m.version DESC
    `
  ...
  try {
    ...
    const modelQuery = `
      ...
      ON CONFLICT (model_id) DO UPDATE SET
        owner = COALESCE(NULLIF(EXCLUDED.owner, ''), models.owner),
        creator = COALESCE(NULLIF(EXCLUDED.creator, ''), models.creator),
        name = COALESCE(NULLIF(EXCLUDED.name, ''), NULLIF(EXCLUDED.name, 'Model #' || models.model_id::text), models.name),
        uri = COALESCE(NULLIF(EXCLUDED.uri, ''), models.uri),
        price_perpetual = CASE WHEN EXCLUDED.price_perpetual = '0' THEN models.price_perpetual ELSE EXCLUDED.price_perpetual END,
        price_subscription = CASE WHEN EXCLUDED.price_subscription = '0' THEN models.price_subscription ELSE EXCLUDED.price_subscription END,
        default_duration_days = CASE WHEN EXCLUDED.default_duration_days = 0 THEN models.default_duration_days ELSE EXCLUDED.default_duration_days END,
        royalty_bps = CASE WHEN EXCLUDED.royalty_bps = 0 THEN models.royalty_bps ELSE EXCLUDED.royalty_bps END,
        delivery_rights_default = CASE WHEN EXCLUDED.delivery_rights_default = 0 THEN models.delivery_rights_default ELSE EXCLUDED.delivery_rights_default END,
        delivery_mode_hint = CASE WHEN EXCLUDED.delivery_mode_hint = 0 THEN models.delivery_mode_hint ELSE EXCLUDED.delivery_mode_hint END,
        terms_hash = COALESCE(EXCLUDED.terms_hash, models.terms_hash),
        price_inference = CASE WHEN EXCLUDED.price_inference = '0' THEN models.price_inference ELSE EXCLUDED.price_inference END,
        inference_wallet = COALESCE(NULLIF(EXCLUDED.inference_wallet, ''), models.inference_wallet),
        inference_endpoint = COALESCE(EXCLUDED.inference_endpoint, models.inference_endpoint),
        agent_id = COALESCE(EXCLUDED.agent_id, models.agent_id),
        listed = true,
        updated_at = NOW()
      RETURNING model_id
    `
