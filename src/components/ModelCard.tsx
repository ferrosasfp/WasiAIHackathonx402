...
export function ModelCard({ locale, data, href: hrefProp, showConnect, priority, onMeta, preMeta }: { locale: string; data: ModelCardData; href?: string; showConnect?: boolean; priority?: boolean; onMeta?: (id: string, meta: any) => void; preMeta?: any }) {
  ...
  // OPTIMIZED: All data should come from Neon DB (indexed API)
  // NO IPFS fetch needed - if data is missing, it should be indexed on model creation/update
  React.useEffect(() => {
    // Only try to extract cover from preMeta if we don't have one from props
    if (coverSrc) return
    if (preMeta?.image || preMeta?.image_url || preMeta?.thumbnail || preMeta?.cover?.cid) {
      const img = preMeta?.image || preMeta?.image_url || preMeta?.thumbnail || preMeta?.cover?.thumbCid || preMeta?.cover?.cid
      if (typeof img === 'string') setCoverSrc(toHttpFromIpfs(img))
    }
    // NOTE: We intentionally do NOT fetch from IPFS here
    // All metadata should be pre-indexed in Neon DB when model is created/updated
  }, [coverSrc, preMeta, toHttpFromIpfs])
